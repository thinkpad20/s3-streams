...
module Network.AWS.S3.Command where
...
instance AwsCfg S3Command where
  getCon = s3Connection
...
buildCommand con steps = buildCommand' con steps' where
  steps' = steps >> addAmzDateHeader >> addContentShaHeader
  
-- | Builds a command in the AWS monad.
buildCommandM :: (Str s, MonadIO io, Functor io)
              => S3Builder s io a -> AwsT s io (S3Command s)
buildCommandM steps = ask >>= \con -> lift $ buildCommand con steps
...
-- | Produces a base s3 command in the AwsT monad.
baseS3CommandM :: (Str s, Monad m, Functor m) => AwsT s m (S3Command s)
baseS3CommandM = fmap baseS3Command ask
